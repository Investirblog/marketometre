...
def fetch_putcall():
    ...
    Le CSV a des lignes comme: 04/25/2026,1234567,2345678,3456789,0.65
    """
    url = 'https://cdn.cboe.com/resources/options/volume_and_call_put_ratios/equitypc.csv'
    r = requests.get(url, timeout=20, headers={'User-Agent': 'Mozilla/5.0'})
    r.raise_for_status()
    lines = r.text.strip().split('\n')
    print(f'  CBOE CSV: {len(lines)} lignes, dernière: {lines[-1][:50]}')

    # Cherche la ligne la plus récente avec une date > 2024
    for line in reversed(lines):
        parts = line.strip().split(',')
        if len(parts) < 5:
            continue
        d_str = parts[0].strip()
        # Format attendu: MM/DD/YYYY ou YYYY-MM-DD
        try:
            # Essai format MM/DD/YYYY
            if '/' in d_str:
                m, d, y = d_str.split('/')
                y = int(y)
            elif '-' in d_str:
    ...
    print('  [CBOE] fallback 0.70')
    return 0.70, 'n/a'

# ── EU SENTIMENT : Consumer Confidence EU via FRED ───────
def fetch_eu_sentiment():
    """
    Consumer Confidence Zone Euro — FRED série EUCSENT.
    Série publiée mensuellement par la Commission Européenne via FRED.
    """
    # Séries FRED validées pour le sentiment EU
    candidates = [
        ('EUCSENT',    1.0),   # EC Consumer Confidence EU, range -40/+5 → ×10
        ('CSCICP03EZM665S', 4.0),  # OCDE Euro Area Consumer Confidence
    ]
    for series, multiplier in candidates:
        try:
            v, d = fred(series)
            scaled = v * multiplier
            print(f'  EU Sentiment ({series}) = {v} → {scaled:.1f} ({d})')
            return scaled, d
        except Exception as e:
            print(f'  [EU Sentiment] {series}: {e}')

    # Fallback: utiliser le score momentum EU comme proxy sentiment
...
def main():
    ...
    data = {
        ...
        'kpis': {
            ...
            'vstoxx': {'value': f'{vstoxx_val:.1f}', 'date': today_str, 'stress': vstoxx_val > 22},
