...
def fetch_eu_sentiment():
    """
    Sentiment EU via l'API publique de la Commission Européenne (DG ECFIN).
    Economic Sentiment Indicator (ESI) — publié mensuellement, 100 = moyenne long terme.
    Complètement gratuit, pas de clé requise.
    Range typique : 70 (crise) à 115 (boom). On normalise vers -100/+100.
    """
    try:
        # API ECFIN Business and Consumer Surveys
        url = 'https://ec.europa.eu/economy_finance/db_indicators/surveys/documents/series/nace2_ecfin_2310/ei_bssi_m_r2.zip'
        # L'API JSON directe est plus simple
        url2 = 'https://ec.europa.eu/economy_finance/db_indicators/surveys/documents/series/nace2_ecfin_2310/esi_ea.csv'
        ...
        if r.ok:
            ...
            v = float(last[-1])
            d = last[0]
            # ESI 100 = neutre, normaliser vers -100/+100
            v_norm = (v - 100) * 5  # 106 → +30, 94 → -30
            print(f'  ESI EU = {v} → scaled {v_norm:.1f} ({d})')
            return v_norm, d
    except Exception as e:
        print(f'  [ESI] API ECFIN échouée: {e}')

    # Fallback FRED — ifo Business Climate Germany (disponible sur FRED)
    try:
        v, d = fred('GFDBUSIDM052NWDB')
        v_norm = (v - 100) * 3
        print(f'  IFO Germany = {v} → scaled {v_norm:.1f} ({d})')
        return v_norm, d
    except Exception as e:
        print(f'  [IFO] FRED échoué: {e}')
...
def fetch_vstoxx():
    """
    VSTOXX — utilise PPFB.DE (ETF Lyxor sur VSTOXX futures) comme proxy fiable,
    ou calcule la volatilité implicite depuis les options Eurostoxx si disponible.
    """
    # PPFB.DE est un ETF coté qui suit le VSTOXX futures — fiable sur Yahoo
    for sym in ['PPFB.DE', 'EVOL.PA', 'FVX', '^VDAX-NEW']:
        try:
            t = yf.Ticker(sym)
            ...
            if len(closes) > 0:
                ...
                # PPFB.DE est en % déjà (ex: 18.5 = VSTOXX à 18.5)
                print(f'  VSTOXX proxy ({sym}) = {v:.2f}')
                return v
        except Exception as e:
            print(f'  [VSTOXX] {sym}: {e}')

    # Dernier recours : calculer la vol réalisée 20j de l'Eurostoxx comme proxy
    try:
        t = yf.Ticker('^STOXX50E')
        h = t.history(period='3mo', auto_adjust=True)
        closes = h['Close'].dropna().tolist()
        if len(closes) >= 20:
            import numpy as np
            returns = [math.log(closes[i]/closes[i-1]) for i in range(-20, 0)]
            vol_20d = (sum(r**2 for r in returns) / 20) ** 0.5 * math.sqrt(252) * 100
            print(f'  VSTOXX (vol réalisée 20j proxy) = {vol_20d:.2f}')
            return vol_20d
    except Exception as e:
        print(f'  [VSTOXX vol proxy] {e}')

    raise ValueError('VSTOXX: toutes les sources ont échoué')
...
def main():
    ...
    zew_val, zew_date   = safe(fetch_eu_sentiment,             (-17.2,'n/a'), 'EU Sentiment')
