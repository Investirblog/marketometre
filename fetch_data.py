...
def fred_zew():
    """
    ZEW Sentiment — essaie plusieurs sources par ordre de fiabilité.
    1. FRED (peut avoir 1-2 jours de retard après la release)
    2. OECD API (gratuit, public, pas de clé requise)
    3. Fallback sur dernière valeur connue
    """
    # Essai 1 : FRED
    try:
        v, d = fred('ZEWSELMU')
        if v != 0.0:  # 0.0 exact = probablement pas encore mis à jour
            return v, d
        print('  [ZEW] FRED retourne 0.0, essai OECD...')
    except Exception as e:
        print(f'  [ZEW] FRED échoué: {e}, essai OECD...')

    # Essai 2 : OECD API (données ZEW Allemagne, série CS-SENTIX-GBR n'est pas ZEW)
    # On utilise l'API Bundesbank qui publie le ZEW rapidement
    try:
        url = 'https://api.bundesbank.de/service/data/BBDE1/M..ZEW.ST.PPOS?startPeriod=2025-01&format=json'
        r = requests.get(url, timeout=15, headers={'Accept': 'application/json'})
        if r.ok:
            j = r.json()
            # Parcourir les observations pour trouver la plus récente
            series = j['data']['dataSets'][0]['series']
            obs_data = list(series.values())[0]['observations']
            periods = j['data']['structure']['dimensions']['observation'][0]['values']
            # Prendre la dernière observation non nulle
            for idx in sorted(obs_data.keys(), key=int, reverse=True):
                val = obs_data[idx][0]
                if val is not None:
                    period = periods[int(idx)]['id']
                    print(f'  [ZEW] Bundesbank = {val} ({period})')
                    return float(val), period
    except Exception as e:
        print(f'  [ZEW] Bundesbank échoué: {e}')

    # Essai 3 : valeur FRED même si 0 (neutre est une valeur valide)
    try:
        v, d = fred('ZEWSELMU')
        print(f'  [ZEW] FRED fallback: {v} ({d})')
        return v, d
    except Exception as e:
        print(f'  [ZEW] Tous les fetch ZEW ont échoué: {e}')
        return 0.0, 'n/a'
