...
def fred_zew():
    """
    ZEW Sentiment via FRED — série DEUODZ066MLONG (ZEW Economic Sentiment Germany).
    Valeur entre -100 (très pessimiste) et +100 (très optimiste).
    """
    # Essai 1 : série principale ZEW sur FRED
    for series in ['DEUODZ066MLONG', 'DEUCSCICP02GYSAM', 'GRZEWI']:
        try:
            v, d = fred(series)
            print(f'  ZEW ({series}) = {v} ({d})')
            return v, d
        except Exception as e:
            print(f'  [ZEW] {series} échoué: {e}')

    # Fallback : dernière valeur connue
    print('  [ZEW] fallback: -17.2 (avril 2026)')
    return -17.2, '2026-04-22'
...
def main():
    ...
    vstoxx_val          = safe(lambda: yf_latest('V2TX.DE'),    22.0,          'VSTOXX')
