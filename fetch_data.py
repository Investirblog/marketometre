...
def fred_zew():
    """ZEW — série correcte sur FRED : GRZEWI (Germany ZEW Index)"""
    try:
        v, d = fred('GRZEWI')
        print(f'  ZEW (GRZEWI) = {v} ({d})')
        return v, d
    except Exception as e:
        print(f'  [ZEW] FRED GRZEWI échoué: {e}')
        # Fallback : FRED série alternative
        try:
            v, d = fred('DISCONTINUED_ZEWSELMU')
            return v, d
        except:
            pass
        # Valeur de secours — ZEW avril 2026 publié le 22/04/2026
        print('  [ZEW] fallback: -17.2')
