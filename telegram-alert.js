...
// Envoie au canal public @marketometre
// Pour envoyer à ton chat perso aussi, ajoute '8712054832' dans TG_CHATS
const TG_CHATS = ['@marketometre', '8712054832'];
...
// ── Telegram — envoie vers tous les destinataires ──
async function sendTelegram(message){
  for(const chatId of TG_CHATS){
    ...
    const j=await r.json();
    if(!j.ok) console.warn(`[Telegram] Erreur pour ${chatId}: ${j.description}`);
    else console.log(`[Telegram] Envoyé à ${chatId}`);
  }
}
