...
function zoneEmoji(s){
  if(s<15) return '🟢';   // panique = opportunité = vert
  if(s<30) return '🟢';   // peur = opportunité
  if(s<45) return '🟡';   // prudence = ambre
  if(s<55) return '⚪';   // neutre
  if(s<70) return '🔵';   // optimisme = bleu
  if(s<85) return '🟠';   // euphorie = orange attention
  return '🔴';             // euphorie extrême = danger
}
